...
<?php
  if (isset($_POST["submit"])) {
	$file = fopen("Sticky_note556.txt", "a");
...
<body>
    <!-- Display all notes -->
    <?php
    if (file_exists("Sticky_note556.txt")) {
        readfile("Sticky_note556.txt");
    }
    ?>
    ...
    <div class="post">
        ...
        <div class="notefoot">
            <?php
            if (isset($_POST["submit"]) && file_exists("Sticky_note556.txt")) {
                $file = fopen("Sticky_note556.txt", "r");
                if ($file) {
                    $content = fread($file, filesize("Sticky_note556.txt"));
                    fclose($file);
